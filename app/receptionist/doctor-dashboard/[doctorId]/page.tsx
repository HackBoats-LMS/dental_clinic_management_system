  ...
  patient: {
    ...
    visits?: {
      treatmentPlan?: {
        ...
        sessions: { sessionNumber: number; status: string; scheduledDate: string }[];
      } | null;
    }[];
  };
  familyMember?: {
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl relative text-left" style={{ maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="space-y-5 overflow-y-auto pr-1 py-4 flex-1 min-h-0">
              ...
              {(() => {
                const visits = selectedVisit.familyMember ? selectedVisit.familyMember.visits : selectedVisit.patient.visits;
                const activePlans = visits?.filter(v => v.treatmentPlan?.status === "PENDING" || v.treatmentPlan?.status === "ACTIVE").map(v => v.treatmentPlan) || [];
                
                if (activePlans.length === 0) return null;
                
                return (
                  <div className="space-y-2 pt-2">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-teal-700 flex items-center gap-2">
                    ...
                    <div className="grid gap-3">
                      {activePlans.map((plan, idx) => {
                        ...
                        return (
                          <div key={idx} className="bg-teal-50 border border-teal-200 p-3.5 rounded-xl flex items-center justify-between">
                            <div>
                              <p className="text-sm font-bold text-slate-900">{plan.selectedProcedure}</p>
                              <p className="text-[10px] text-slate-500 font-semibold tracking-wider uppercase mt-0.5">
                                Session {completedSessions} of {plan.totalSessions} completed
                              </p>
                            </div>
                            <span className="px-2.5 py-1 bg-white border border-teal-200 text-teal-700 text-[10px] font-bold rounded-lg uppercase tracking-wider shadow-xs">
                              {plan.status}
                            </span>
                          </div>
                        );
